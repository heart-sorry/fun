...
(function () {
    ...
    const defaultTasks = {
        ...
        // 📋 求职准备（不考研路线）
        //
        // 核心理念：项目 > 学历，GitHub > 简历
        // 大一打基础，大二找实习，大三冲秋招
        // ═══════════════════════════════════════
        career: [
            {
                section: '📋 求职准备 — 阶段一：基础积累（大一 → 大二上）',
                items: [
                    { text: 'GitHub 个人主页建设（头像 / Bio / Pinned 项目）', path: 'GitHub Profile README 教程' },
                    { text: '每个项目写清楚 README（背景 + 架构 + 效果图 + 技术栈）', path: 'makeareadme.com / README 模板' },
                    { text: '电赛参赛（省赛起步，冲国奖）', path: '全国大学生电子设计竞赛官网' },
                    { text: '至少完成 3 个可演示的完整项目（不是课程作业）', path: '项目灵感：awesome-embedded / awesome-robotics' },
                    { text: '写技术博客 / 学习笔记（CSDN / 掘金 / 个人博客）', path: '技术写作提升面试通过率' },
                    { text: '加入实验室或技术社团（接触真实项目）', path: '联系学院老师 / 学校机器人队' },
                ]
            },
            {
                section: '📋 求职准备 — 阶段二：实习冲刺（大二下 → 大三上）',
                items: [
                    { text: '制作一页纸简历（项目经历 > 课程 > GPA）', path: '牛客网简历模板 / LaTeX 简历' },
                    { text: '刷 LeetCode（侧重 C++ / Python，每天 1-2 题）', path: 'LeetCode Hot 100 / 代码随想录' },
                    { text: '大二下开始投实习（嵌入式 / 机器人 / 边缘 AI 方向）', path: '牛客网 / BOSS直聘 / 实习僧' },
                    { text: '目标公司：大疆 / 宇树 / 智元 / 小鹏 / 追觅 / 科大讯飞', path: '各公司招聘官网 / 校招日历' },
                    { text: '面试八股文（C语言 / 操作系统 / 计算机网络 / 嵌入式）', path: '面经搜索：牛客网面经区' },
                    { text: '争取 1-2 段实习经历（哪怕是小公司也值得）', path: '实习经验 > 没有实习的研究生' },
                ]
            },
            {
                section: '📋 求职准备 — 阶段三：秋招备战（大三下 → 大四上）',
                items: [
                    { text: '整理 GitHub 精选项目（3 个最能打的，写好文档 + Demo 视频）', path: '项目演示视频 > 截图' },
                    { text: '准备自我介绍 + 项目深挖（STAR 法则讲项目）', path: 'STAR 法则：Situation / Task / Action / Result' },
                    { text: '模拟面试（找同学互面 / 牛客模拟面）', path: '牛客模拟面试 / 牛客讨论区' },
                    { text: '投递策略：提前批（6-7月）→ 正式批（8-9月）→ 补录（10-11月）', path: '校招时间线 / 各公司提前批公告' },
                    { text: '拿到Offer后谈薪（了解市场价 + 多Offer对比）', path: '脉脉 / 牛客薪资爆料' },
                ]
            },
            {
                section: '📋 求职准备 — 目标岗位对照',
                items: [
                    { text: '嵌入式软件工程师 — 要求：C/C++ + STM32 + RTOS（对应硬件线）', path: '你硬件线阶段二到四全覆盖' },
                    { text: '机器人软件工程师 — 要求：ROS2 + C++/Python + 传感器（对应硬件+具身）', path: '你硬件线 + 具身线交叉覆盖' },
                    { text: '边缘 AI 部署工程师 — 要求：ONNX/TensorRT + 量化 + 嵌入式（对应AI+硬件）', path: '你AI线阶段四五 + 硬件线' },
                    { text: '机器人测试工程师 — 要求：ROS2 + Python + 自动化（门槛最低，保底选择）', path: '你硬件线 + AI线阶段一二即可入门' },
                ]
            },
        ]
    };
